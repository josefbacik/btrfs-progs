...
again:
	...
	if (ret < 0)
		goto out;
	ret = load_super_root(&normal_trees, gfs_info->remap_root);
	if (ret < 0)
		goto out;
...
{
	struct btrfs_root *root = btrfs_get_block_group_root(gfs_info);
	u64 start = 0;
	int ret;
...
static int repair_root_items(void)
{
	...
	/*
	 * If we are v2 then we don't have entries for our roots in the extent
	 * tree, just return.
	 */
	if (btrfs_fs_incompat(gfs_info, EXTENT_TREE_V2))
		return 0;
