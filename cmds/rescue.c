...
static const char * const cmd_rescue_recover_chunks_usage[] = {
	"btrfs rescue recover-chunks <device>",
	"Search the system blocks looking for missing chunks",
	NULL
};

static int cmd_rescue_recover_chunks(const struct cmd_struct *cmd, int argc,
				     char *argv[])
{
	char *devname;
	int ret;
	...
	ret = btrfs_find_recover_chunks(devname);
	if (!ret) {
		fprintf(stdout, "Recover chunks succeeded, you can run check now\n");
	} else {
		fprintf(stdout, "Recover chunks tree failed\n");
	}
	return ret;
}
static DEFINE_SIMPLE_COMMAND(rescue_recover_chunks, "recover-chunks");
...
static int cmd_rescue_tree_recover(const struct cmd_struct *cmd,
				   int argc, char *argv[])
{
	char *devname;
	int ret;
	...
	ret = btrfs_recover_trees(devname);
	if (!ret) {
		fprintf(stdout, "Tree recovery finished, you can run check now\n");
	} else {
		fprintf(stdout, "Tree recover failed\n");
	}
	return ret;
}
static DEFINE_SIMPLE_COMMAND(rescue_tree_recover, "tree-recover");
...
static const struct cmd_group rescue_cmd_group = {
	rescue_cmd_group_usage, rescue_cmd_group_info, {
		...
		&cmd_struct_rescue_recover_chunks,
		NULL
	}
};
