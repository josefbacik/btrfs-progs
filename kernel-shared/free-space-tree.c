...
static struct btrfs_root *get_free_space_root(struct btrfs_fs_info *fs_info,
				      struct btrfs_block_group *block_group)
{
	if (!btrfs_fs_incompat(fs_info, EXTENT_TREE_V2))
		return fs_info->_free_space_root;
	return block_group->free_space_root;
}
...
{
	struct btrfs_root *root = get_free_space_root(fs_info, block_group);
...
{
	struct btrfs_root *root = get_free_space_root(trans->fs_info,
						      block_group);
