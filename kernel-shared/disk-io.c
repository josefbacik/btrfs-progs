				      ...
				      struct btrfs_key *location)
{
	...
	if (location->objectid == BTRFS_REMAP_TREE_OBJECTID)
		return fs_info->remap_root ? fs_info->remap_root :
						ERR_PTR(-ENOENT);
...
struct btrfs_fs_info *btrfs_new_fs_info(int writable, u64 sb_bytenr)
{
	...
	if (!fs_info->tree_root || !fs_info->_extent_root ||
	    ...
	    !fs_info->remap_root || !fs_info->super_copy)
		goto free_all;
				...
				u64 root_tree_bytenr, unsigned flags)
{
	...
	if (backup) {
		bytenr = btrfs_backup_remap_root(backup);
		gen = btrfs_backup_remap_root_gen(backup);
	} else {
	...
	root->node = read_tree_block(fs_info, bytenr, gen);
	if (!extent_buffer_uptodate(root->node)) {
		fprintf(stderr, "Couldn't read remap root\n");
		return -EIO;
	}
			  ...
			  unsigned flags)
{
	...
	if (btrfs_fs_incompat(fs_info, EXTENT_TREE_V2)) {
		...
		ret = btrfs_read_block_group_roots(fs_info);
		if (ret)
			return ret;
	}
				     ...
				     struct btrfs_key *key)
{
	...
	ASSERT(key->type == BTRFS_ROOT_ITEM_KEY);
	...
	btrfs_set_header_owner(leaf, key->objectid);
	root->node = leaf;
