			      ...
			      bool extent_tree_v2)
{
	...
	const char *csum_tree_str = "backup_csum_root";

	if (extent_tree_v2) {
		extent_tree_str = "backup_block_group_root";
		csum_tree_str = "backup_remap_root";
	}
...
void btrfs_print_superblock(struct btrfs_super_block *sb, int full)
{
	...
	printf("remap_root\t%llu\n",
	       (unsigned long long)btrfs_super_remap_root(sb));
	printf("remap_root_generation\t%llu\n",
	       (unsigned long long)btrfs_super_remap_root_generation(sb));
	printf("remap_root_level\t%llu\n",
	       (unsigned long long)btrfs_super_remap_root_level(sb));
