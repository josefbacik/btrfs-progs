			     ...
			     struct btrfs_block_group *block_group)
{
	...
	root = btrfs_extent_root(root->fs_info, block_group->start);
...
int btrfs_free_block_groups(struct btrfs_fs_info *info)
{
	...
	rbtree_postorder_for_each_entry_safe(cache, next,
		...
		if (cache->extent_root) {
			free_extent_buffer(cache->extent_root->node);
...
int btrfs_read_block_groups(struct btrfs_fs_info *fs_info)
{
	...
	root = btrfs_get_block_group_root(fs_info);
	key.objectid = 0;
	key.offset = 0;
	key.type = BTRFS_BLOCK_GROUP_ITEM_KEY;
